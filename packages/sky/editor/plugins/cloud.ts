...
export default function createCloud(sky: Sky) {
  ...
  module.updateCloudsElementRect = (
    clouds: Cloud[] = sky.state.targetClouds,
  ) => {
    clouds.forEach(cloud => {
      const el = sky.cloud.queryCloudElementById(cloud.id);
      if (!el) return;
  ...
  module.queryCloudElementById = id => {
    return sky.vm.$el.querySelector(`[data-cloud-id="${id}"]`);
  };
  ...
  module.setSelectCloud = event => {
    ...
    const oldCloud = sky.state.selectCloud;
    if (oldCloud === cloud) return;

    sky.state.selectCloud = cloud;
  ...
  module.delete = ({ force = false } = {}) => {
    ...
    const targetIds = sky.state.targetClouds.map(cloud => cloud.id);
  ...
  module.copy = () => {
    ...
    sky.state.clipboard = JSON.stringify(sky.state.targetClouds);
    return sky.state.clipboard;
  };

  module.paste = async () => {
    if (!sky.state.clipboard) return;
    ...
    const clouds: Cloud[] = JSON.parse(sky.state.clipboard);
    ...
    const targetCloudsRect =
      sky.state.targetClouds.length > 0
  ...
  module.toGroup = async () => {
    ...
    const targetClouds = [...sky.state.targetClouds];
  ...
  module.moveTop = () => {
    const { clouds, targetClouds } = sky.state;
